...
Generate exactly ONE image containing exactly ONE single specimen — only the midpoint form itself, never a sequence, never a grid, never multiple stages or variants side by side. The single specimen is perfectly centered in the frame, fully visible, with GENEROUS empty background margins on all four sides (the specimen occupies roughly the middle 60% of the frame, never touching or approaching any edge), and the seamless plain PURE WHITE paper backdrop (clean bright white, like #ffffff) — the background must read as white, never grey: not medium grey, not light grey, not off-white fills the entire frame edge to edge with nothing else in it. The backdrop is OVEREXPOSED by the flash to a uniform, blown-out, burnt pure white (#ffffff) across the ENTIRE frame — completely even, with absolutely no gradient, vignette, grey falloff, shadow, or colour tint in any corner or along any edge, so the empty background is a single flat field of maximum white. Only the specimen itself may cast a small hard flash shadow immediately beneath it; the rest of the background stays blown-out white.
