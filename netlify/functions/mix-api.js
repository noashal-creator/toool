...
Generate exactly ONE image containing exactly ONE single specimen — only the midpoint form itself, never a sequence, never a grid, never multiple stages or variants side by side. The single specimen is perfectly centered in the frame, fully visible with clear margins on all sides, and the seamless plain light grey paper backdrop fills the entire frame edge to edge with nothing else in it.`;
