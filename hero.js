/* ─────────────────────────────────────────────────────────────────────
   hero — pinned horizontal spectrum (now: any number of them)

   Scrolling into a band hijacks vertical scroll into a sideways sweep: the
   band is pinned (position:fixed) and fills the screen while its wide strip
   slides across; the page cannot advance until the whole spectrum has been
   swept (~1.2 screens), then normal scrolling resumes.
   ...
   The same logic drives EVERY [data-pan-band] element (cow→tomato #hero and
   the tooth→troll #troll-band): each is an independent instance with its own
   sticky / track / strip image, found by structure so the two bands can carry
   different class names and never share styling or a colour control.
...
(() => {
  const bands = document.querySelectorAll('[data-pan-band]');
  if (!bands.length) return;
  bands.forEach(initBand);

  function initBand(hero) {
    const sticky = hero.querySelector('.hero__sticky, .tband__sticky');
    const track  = hero.querySelector('.hero__track, .tband__track');
    ...
    /* idle auto-scroll — before the user has scrolled the band into its pin
       range, run the whole strip past on its own, like film advancing through a
       projector: one steady direction, constant speed, looping with NO visible
       cut (no back-and-forth tease, no easing at the ends, no jump at the loop
       point). Handed off the instant real scroll engagement begins (onScroll
       below); resumes automatically if the user scrolls back above the band.
       Owns track.style.transform only while running — onScroll always takes it
       back over once p > 0.
    ...
    /* ── sideways: the sweep is the scroll ───────────────────────────────────
       While the reel owns the slide the strip offset is p·travel, and nothing
       else: p is the slide's dwell fraction (0→1), scrubbed by scrolling while
       the slide is held centred, so the spectrum moves exactly as much as you
       scroll and stops exactly where you stop. */
