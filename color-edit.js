...
(() => {
  ...
  const STORE = 'toool-sidebar-colors';
  let saved = {};
  ...
  const CONTROLS = [
    ...
    { key: 'heroBg', name: 'רקע הפרה (ירוק)',
      get: () => { const h = $('.hero__sticky'); return h ? getComputedStyle(h).backgroundColor : '#00961C'; },
      apply: v => $$('.hero__sticky').forEach(h => h.style.backgroundColor = v),
      css: v => '.hero__sticky { background: ' + v + '; }' },
  ...
  function buildPanel() {
    ...
    panel.querySelector('.ce-reset').onclick = () => {
      ...
      $$('.hero__sticky').forEach(h => h.style.removeProperty('background-color'));
